...
if ($func == "product-pos"){
    $sql = "SELECT product_code, product_name, product_type, color, item_price, IFNULL(SUM(quantity),0) AS qty FROM products LEFT JOIN whse_items USING (product_code) WHERE quantity != 0 GROUP BY product_code";
    $result = mysqli_query($con,$sql) or die($con->error); //or die($con->error) is for debugging of SQL Query
    while($rows = mysqli_fetch_array($result)){
        $product_code = $rows['product_code'];
        $product_name = $rows['product_name'];
    ...
    <tr  data-pcode= '<?= $product_code ?>' data-price= '<?= $item_price ?>' id='tr_<?= $product_code ?>' class = 'select_pos_item'>
        <td class = "table-main" ><img src='Images\Backgrounds\bc.jpg' width="50px" alt="Product_image"></td>
        <td class = "table-main" ><?= $product_code ?></td>
        <td class = "table-main" ><?= $product_name ?></td>
        <td class = "table-main" ><?= $product_type ?></td>
        <td class = "table-main" ><?= $qty?></td>
