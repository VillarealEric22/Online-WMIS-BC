<?php
    ...
    if($func == "disp"){
      ...
      $stmt = $con->prepare($sql);
      $stmt->bind_param('ds',  $package_price, $package_code);

      if ($stmt->execute()){
        $sqlIn = "INSERT INTO package_items (product_code, quantity, package_code) VALUES (?,?,?) ON DUPLICATE KEY UPDATE quantity = VALUES(quantity)";
        $stmtIn = $con->prepare($sqlIn);
        foreach ( $mi as $value ) {
          list($product_code, $quantity, $pckgCD) = $value;
          $stmtIn->bind_param('sis', $product_code, $quantity, $pckgCD);
          $stmtIn->execute();
        }
        echo $con->error;
      }
    ...
    else if($func == "delete2"){
        $package_code = $_POST['deleteID'];
        $product_code = $_POST['product_code'];
        $sql = "DELETE FROM package_items WHERE product_code = '$product_code' AND package_code = '$package_code'";
        $result = mysqli_query($con, $sql);
        if ($result === true) {
    ...
    else if($func == "auto_table"){
      $pkgcode = $_POST['pkgcode'];
      $sql = "SELECT product_code, quantity FROM package_items WHERE package_code = '$pkgcode'";
      $result = mysqli_query($con,$sql) or die($con->error); //or die($con->error) is for debugging of SQL Query
      $array = array();
